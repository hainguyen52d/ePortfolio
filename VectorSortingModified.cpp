...
void quickSort(vector<Bid>& bids, int begin, int end) {
    ...
    // Optimization: Use insertion sort for small partitions
    if (end - begin + 1 <= 10) {
        ...
        for (int i = begin + 1; i <= end; ++i) {
            ...
            // Move elements of bids[begin..i-1] that are greater than key.title
            // to one position ahead of their current position
            while (j >= begin && bids[j].title.compare(key.title) > 0) {
                bids[j + 1] = bids[j];
                --j;
            }
        ...
        // Continue with quicksort for larger partitions
...
int main(int argc, char* argv[]) {
    ...
    while (choice != 9) {
        ...
        case 2:
            ...
            // FIXME (1b): Invoke the selection and report timing results.
        case 3:
           // Initialize a timer variable before loading bids
            ticks = clock();
